...
const UpdateCustomerModel = ({
  ...
  const checkPhoneExists = (_, phone) => {
    ...
    if (Array.isArray(customerData) && customerData.length > 0) {
      const phoneExists = customerData.some(
        (cust) => cust.phone === phone && cust._id !== customer._id
      );
      if (phoneExists) {
        return Promise.reject("This phone number is already in use!");
      }
  ...
  const checkAddressExists = (_, address) => {
    if (!address) {
      return Promise.reject("Please input the address of the customer!");
    }
    if (Array.isArray(customerData) && customerData.length > 0) {
      const addressExists = customerData.some(
        (cust) => cust.address === address && cust._id !== customer._id
      );
      if (addressExists) {
        return Promise.reject("This address is already in use!");
      }
    }
    return Promise.resolve();
  };
